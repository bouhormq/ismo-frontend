...
export const SendEmailFormWrapper = ({ handleClose, isSending = false }: Props) => {
  ...
  // Fetch contacts for the first selected company
  const firstCompanyId = selectedCompanies[0]?.id;
  const { data: contactsData } = useQuery({
    queryKey: ["getCompanyContacts", firstCompanyId],
    queryFn: () =>
      getAllCompanyContacts(firstCompanyId!, undefined, {
        limit: 100,
        offset: 0,
      }),
    ...
    >
      ...
      {selectedCompanies.length > 0 && (
        <div className="w-full text-left">
          ...
          <div className="w-full rounded-full border border-gray-300 bg-gray-50 px-4 py-2.5 text-sm text-gray-700">
